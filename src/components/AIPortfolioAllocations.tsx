...
interface AIPortfolioAllocationsProps {
  allocations: Allocation[];
  onSelect: (label: string) => void;
}

export default function AIPortfolioAllocations({ allocations, onSelect }: AIPortfolioAllocationsProps) {
  return (
    <div className="bg-[#141414] border border-white/10 p-8 rounded-[40px] relative overflow-hidden h-full">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h3 className="text-xl font-black tracking-tight">AI Portfolio Allocations</h3>
      ...
      <div className="flex flex-col items-center justify-center py-8 mb-8 relative">
        <svg className="w-48 h-48 transform -rotate-90">
          {allocations.map((a, i) => {
            ...
            const previousPercentage = allocations.slice(0, i).reduce((acc, curr) => acc + (curr.value / total) * 100, 0);
        ...
        {allocations.map((a, i) => (
          ...
          >
            <div className="flex items-center gap-3 mb-1">
              ...
              <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-500">{a.label}</h4>
            </div>
