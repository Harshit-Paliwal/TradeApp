...
import { Shield, Target, Zap, TrendingUp, Activity, AlertCircle } from 'lucide-react';
...
interface AIPortfolioRisksProps {
  score: number;
  metrics: RiskMetric[];
}

export default function AIPortfolioRisks({ score, metrics }: AIPortfolioRisksProps) {
  return (
    <div className="bg-[#141414] border border-white/10 p-8 rounded-[40px] relative overflow-hidden h-full">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h3 className="text-xl font-black tracking-tight">AI Portfolio Risks</h3>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                <div className={cn(
                  "p-2 rounded-xl",
                  m.status === 'low' ? "bg-green-500/20 text-green-500" : 
                  m.status === 'medium' ? "bg-orange-500/20 text-orange-500" : 
                  "bg-red-500/20 text-red-500"
                )}>
                  {m.status === 'low' ? <Shield size={16} /> : 
                   m.status === 'medium' ? <Zap size={16} /> : 
                   <AlertCircle size={16} />}
                </div>
